...
const VoyageView = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const formattedStats = {
        total_days: statsData.overview?.total_days || 0,
        total_countries: statsData.by_country?.length || 0,
        total_destinations: new Set((daysData.days || []).map(d => d.destination)).size,
        total_budget: statsData.overview?.total_budget_hotels || 0,
      ...
      const countries = {};
      (formattedStats.by_country || []).forEach(c => {
  ...
  const loadHotels = async (destination) => {
    ...
    try {
      // Pour les transitions (→), prendre la destination FINALE
      let destName = destination;
      if (destination.includes('→')) {
        destName = destination.split('→').pop();
      }
      
      // Convertir le nom de destination en ID
      const destId = destName.toLowerCase()
        .trim()
        .replace(/\s+/g, '-')
        .replace(/[^a-z0-9-]/g, '');
      
      console.log('Recherche hôtels pour:', destId);
