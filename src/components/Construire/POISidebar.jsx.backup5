...
// Types qui ne sont PAS filtrés par "Type de journée"
const ALWAYS_VISIBLE_TYPES = ['restaurant', 'restaurer', 'bar', 'verre', 'hotel', 'hôtel', 'loger', 'hébergement'];
...
export function POISidebar({ 
  ...
  // Vérifier si un POI est un resto/bar/hôtel (toujours visible)
  const isAlwaysVisible = (poi) => {
    const poiType = `${poi.type || ''} ${poi.category || ''}`.toLowerCase();
    return ALWAYS_VISIBLE_TYPES.some(t => poiType.includes(t));
  };
  ...
  const filteredPOIs = useMemo(() => {
    return pois.filter(poi => {
      ...
      // Type de journée : NE filtre PAS les restos/bars/hôtels
      if (journeyType) {
        // Si c'est un resto/bar/hôtel, on le garde toujours
        if (isAlwaysVisible(poi)) return true;
        
        // Sinon, on applique le filtre type de journée
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <span style={{ 
            ...
            {sortedPOIs.length} lieu{sortedPOIs.length > 1 ? 'x' : ''}
          </span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          {JOURNEY_TYPES.map(type => (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                setJourneyType(journeyType === type.id ? null : type.id);
              }}
              style={{
                flex: 1,
                padding: '10px 14px',
