...
const TransportView = () => {
  ...
  // ============================================
  // ONGLET MON ITINÉRAIRE (FUSIONNÉ AVEC VOLS)
  // ============================================
  const MonItineraireTab = () => {
    ...
    useEffect(() => {
      if (editingTransport) {
        // Détecte si c'est un vol international
        const isFlight = editingTransport._type === 'flight';
        setFormData({
          trip_day: editingTransport.trip_day || '',
          date: isFlight ? editingTransport.departure_date : (editingTransport.date || ''),
          from_place: isFlight ? editingTransport.from_city : (editingTransport.from_place || ''),
          to_place: isFlight ? editingTransport.to_city : (editingTransport.to_place || ''),
          transport_type: isFlight ? 'avion' : (editingTransport.transport_type || 'grab'),
          transport_provider: isFlight ? editingTransport.airline : (editingTransport.transport_provider || ''),
          departure_time: editingTransport.departure_time || '',
          duration_hours: editingTransport.duration_hours || '',
          cost_eur: isFlight ? editingTransport.price_eur : (editingTransport.cost_eur || ''),
          booking_status: editingTransport.booking_status || 'a_planifier',
          notes: editingTransport.notes || '',
          is_international_flight: isFlight,
          from_airport_code: isFlight ? editingTransport.from_airport_code : '',
          to_airport_code: isFlight ? editingTransport.to_airport_code : ''
        });
      }
    }, [editingTransport]);
    ...
    const resetForm = () => {
      setFormData({
        ...
        is_international_flight: false, from_airport_code: '', to_airport_code: ''
      });
    };
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        if (formData.is_international_flight) {
          // Appel API pour vol international
          const flightData = {
            from_city: formData.from_place,
            to_city: formData.to_place,
            from_airport_code: formData.from_airport_code || formData.from_place.substring(0, 3).toUpperCase(),
            to_airport_code: formData.to_airport_code || formData.to_place.substring(0, 3).toUpperCase(),
            departure_date: formData.date,
            departure_time: formData.departure_time,
            duration_hours: formData.duration_hours ? parseFloat(formData.duration_hours) : null,
            price_eur: formData.cost_eur ? parseFloat(formData.cost_eur) : null,
            airline: formData.transport_provider,
            booking_status: formData.booking_status,
            notes: formData.notes,
            trip_day: formData.trip_day ? parseInt(formData.trip_day) : null
          };

          const url = editingTransport && editingTransport._type === 'flight'
            ? `${API_URL}/api/international-flights/${editingTransport.id}`
            : `${API_URL}/api/international-flights`;
          
          const res = await fetch(url, {
            method: editingTransport && editingTransport._type === 'flight' ? 'PUT' : 'POST',
          ...
          // Appel API pour transport normal
          const url = editingTransport && editingTransport._type !== 'flight'
    ...
    const handleDelete = async (transport) => {
      ...
      try {
        const url = transport._type === 'flight'
          ? `${API_URL}/api/international-flights/${transport.id}`
          : `${API_URL}/api/my-transports/${transport.id}`;
    ...
    const getTypeIcon = (type, isFlight) => {
      if (isFlight) return Icons.plane;
    ...
    // Fusionner vols et transports avec un marqueur de type
    const allTransports = [
      ...flights.map(f => ({
        ...f,
        _type: 'flight',
        _sortDay: f.trip_day || (f.departure_date ? getDayFromDate(f.departure_date) : 999),
        _sortTime: f.departure_time || '00:00'
      })),
      ...myTransports.map(t => ({
        ...t,
        _type: 'transport',
        _sortDay: t.trip_day || 999,
        _sortTime: t.departure_time || '00:00'
      }))
    ];

    // Fonction pour calculer le jour à partir d'une date
    ...
    // Grouper par jour
    const groupedByDay = allTransports.reduce((acc, t) => {
      const day = t._sortDay;
      if (!acc[day]) acc[day] = [];
      acc[day].push(t);
      return acc;
    }, {});

    // Trier les transports dans chaque jour par heure
    Object.keys(groupedByDay).forEach(day => {
      groupedByDay[day].sort((a, b) => (a._sortTime || '').localeCompare(b._sortTime || ''));
    });

    const sortedDays = Object.keys(groupedByDay).map(Number).sort((a, b) => a - b);

    // Calcul des totaux
    const totalCost = allTransports.reduce((sum, t) => {
      if (t._type === 'flight') return sum + (t.price_eur || 0);
      return sum + (t.cost_eur || 0);
    }, 0);

    const flightsCount = allTransports.filter(t => t._type === 'flight').length;
    const transportsCount = allTransports.filter(t => t._type === 'transport').length;
    ...
    return (
      <div style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <span style={{ background: '#f0fdfa', color: '#0d9488', padding: '4px 12px', borderRadius: '12px', fontSize: '14px' }}>
              {allTransports.length} trajets
            </span>
            {flightsCount > 0 && (
              <span style={{ background: '#ede9fe', color: '#7c3aed', padding: '4px 12px', borderRadius: '12px', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                {Icons.plane} {flightsCount} vols
              </span>
            )}
          ...
          <button
            onClick={() => { setShowForm(true); setEditingTransport(null); resetForm(); }}
            style={{
        ...
        {showForm && (
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={() => { setShowForm(false); setEditingTransport(null); resetForm(); }}
            ...
            <form onSubmit={handleSubmit}>
              {/* Checkbox Vol International */}
              <div style={{ 
                marginBottom: '20px', padding: '16px', borderRadius: '10px',
                background: formData.is_international_flight ? '#ede9fe' : '#f9fafb',
                border: formData.is_international_flight ? '2px solid #7c3aed' : '1px solid #e5e7eb'
              }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}>
                  <input 
                    type="checkbox" 
                    checked={formData.is_international_flight}
                    onChange={(e) => setFormData({
                      ...formData, 
                      is_international_flight: e.target.checked,
                      transport_type: e.target.checked ? 'avion' : 'grab'
                    })}
                    style={{ width: '20px', height: '20px', cursor: 'pointer' }}
                  />
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ color: '#7c3aed' }}>{Icons.globe}</span>
                    <span style={{ fontWeight: '600', fontSize: '15px' }}>Vol international</span>
                  </div>
                  <span style={{ fontSize: '13px', color: '#6b7280' }}>
                    (vol entre pays : France ↔ Philippines, Philippines → Thaïlande, etc.)
                  </span>
                </label>
              </div>
              ...
              {formData.is_international_flight && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '16px' }}>
                  <div>
                    <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '500' }}>Code aéroport départ</label>
                    <input type="text" value={formData.from_airport_code} maxLength="3"
                      onChange={(e) => setFormData({...formData, from_airport_code: e.target.value.toUpperCase()})}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d5db', fontSize: '14px' }}
                      placeholder="Ex: CDG, MNL, BKK"
                    />
                  </div>
                  <div>
                    <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '500' }}>Code aéroport arrivée</label>
                    <input type="text" value={formData.to_airport_code} maxLength="3"
                      onChange={(e) => setFormData({...formData, to_airport_code: e.target.value.toUpperCase()})}
                      style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d5db', fontSize: '14px' }}
                      placeholder="Ex: CDG, MNL, BKK"
                    />
                  </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '16px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '500' }}>
                    {formData.is_international_flight ? 'Ville départ *' : 'Départ *'}
                  </label>
                  <input type="text" value={formData.from_place} required
                    onChange={(e) => setFormData({...formData, from_place: e.target.value})}
                    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d5db', fontSize: '14px' }}
                    placeholder={formData.is_international_flight ? 'Ex: Paris' : 'Ex: Hôtel Makati'}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '500' }}>
                    {formData.is_international_flight ? 'Ville arrivée *' : 'Arrivée *'}
                  </label>
                  <input type="text" value={formData.to_place} required
                    onChange={(e) => setFormData({...formData, to_place: e.target.value})}
                    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d5db', fontSize: '14px' }}
                    placeholder={formData.is_international_flight ? 'Ex: Manille' : 'Ex: Aéroport MNL'}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '16px' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '6px', fontSize: '13px', fontWeight: '500' }}>
                    {formData.is_international_flight ? 'Compagnie' : 'Type *'}
                  </label>
                  {formData.is_international_flight ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '16px', marginBottom: '20px' }}>
                ...
                <div>
                  ...
                  <input type="text" value={formData.notes}
                    ...
                    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d5db', fontSize: '14px' }}
                    placeholder={formData.is_international_flight ? 'Ex: Escale probable, Vol direct' : 'Ex: Réserver la veille'}
                  />
                </div>
              </div>

              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                <button type="button" onClick={() => { setShowForm(false); setEditingTransport(null); resetForm(); }}
                ...
                <button type="submit"
                  style={{ 
                    ...
                    background: formData.is_international_flight ? '#7c3aed' : '#14B8A6', 
          ...
          <div key={day} style={{ marginBottom: '24px' }}>
            <div style={{
              ...
              <span style={{ fontWeight: '600', fontSize: '16px' }}>
                {day === 999 ? 'Date à définir' : `Jour ${day}`}
              </span>
              {groupedByDay[day][0]?.date && (
                <span style={{ color: '#6b7280', fontSize: '14px' }}>
                  - {new Date(groupedByDay[day][0].date || groupedByDay[day][0].departure_date).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'short' })}
                </span>
              )}
            </div>
            ...
            {groupedByDay[day].map((transport) => {
              ...
              const isFlight = transport._type === 'flight';
              const fromText = isFlight ? transport.from_city : transport.from_place;
              const toText = isFlight ? transport.to_city : transport.to_place;
              const cost = isFlight ? transport.price_eur : transport.cost_eur;
              const provider = isFlight ? transport.airline : transport.transport_provider;
              const time = isFlight ? transport.departure_time : transport.departure_time;

              return (
                <div key={`${transport._type}-${transport.id}`} style={{
                  background: 'white', 
                  border: isFlight ? '2px solid #7c3aed' : '1px solid #e5e7eb', 
                  borderRadius: '10px',
                  padding: '16px', marginBottom: '8px', marginLeft: '20px',
                  display: 'flex', alignItems: 'center', gap: '16px'
                }}>
                  <div style={{ 
                    color: isFlight ? '#7c3aed' : '#14B8A6', 
                    background: isFlight ? '#ede9fe' : '#f0fdfa', 
                    padding: '10px', borderRadius: '8px' 
                  }}>
                    {getTypeIcon(transport.transport_type, isFlight)}
                  </div>

                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                      {time && (
                        <span style={{ fontWeight: '600', color: isFlight ? '#7c3aed' : '#14B8A6' }}>{time}</span>
                      )}
                      {isFlight && transport.from_airport_code && (
                        <span style={{ fontWeight: '700', fontSize: '16px' }}>{transport.from_airport_code}</span>
                      )}
                      <span style={{ fontWeight: '500' }}>{fromText}</span>
                      <span style={{ color: '#9ca3af' }}>{Icons.arrowRight}</span>
                      {isFlight && transport.to_airport_code && (
                        <span style={{ fontWeight: '700', fontSize: '16px' }}>{transport.to_airport_code}</span>
                      )}
                      <span style={{ fontWeight: '500' }}>{toText}</span>
                    </div>
                    <div style={{ fontSize: '13px', color: '#6b7280', display: 'flex', gap: '16px', alignItems: 'center' }}>
                      {isFlight && (
                        <span style={{ background: '#ede9fe', color: '#7c3aed', padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: '600' }}>
                          VOL INTERNATIONAL
                        </span>
                      )}
                      {!isFlight && <span style={{ textTransform: 'capitalize' }}>{transport.transport_type}</span>}
                  ...
                  <div style={{ textAlign: 'right', minWidth: '80px' }}>
                    ...
                    {cost > 0 && (
                      <div style={{ color: isFlight ? '#7c3aed' : '#14B8A6', fontWeight: '500' }}>{cost}€</div>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => handleDelete(transport)}
        ...
        {allTransports.length === 0 && !loading && (
  ...
  // ============================================
  // RENDU PRINCIPAL (3 onglets au lieu de 4)
  // ============================================
  return (
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {loading ? (
          ...
          <>
            ...
            {activeTab === 'connexions' && <ConnexionsTab />}
          </>
        )}
      </div>
    </div>
