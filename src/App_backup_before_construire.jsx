...
const styles = {
  sidebar: { width: '64px', background: '#ffffff', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '16px', gap: '4px' },
  sidebarBtn: (active) => ({ width: '48px', height: '48px', borderRadius: '8px', border: 'none', background: active ? '#eef2ff' : 'transparent', color: active ? '#6366f1' : '#6b7280', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2px', fontSize: '9px', fontWeight: 500, transition: 'all 0.15s' }),
...
export default function App() {
  ...
  const explorerPois = POI_DATA[explorerCountry]?.[explorerDest] || [];
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <aside style={styles.sidebar}>
        <div style={{ width: '36px', height: '36px', background: '#6366f1', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '14px', marginBottom: '16px' }}>TF</div>
        {[
          { id: 'plan', icon: 'calendar', label: 'Plan' },
          { id: 'budget', icon: 'wallet', label: 'Budget' },
          { id: 'favorites', icon: 'star', label: 'Favoris' },
          { id: 'explore', icon: 'search', label: 'Explorer' },
          { id: 'guide', icon: 'map', label: 'Guide' },
        ].map(item => (
          <button key={item.id} onClick={() => setSidebarTab(item.id)} style={styles.sidebarBtn(sidebarTab === item.id)}>
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: '24px' }}>
            ...
            {favorites.length === 0 ? (
              <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '60px', textAlign: 'center' }}>
                <div style={{ width: '48px', height: '48px', background: '#f1f5f9', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
